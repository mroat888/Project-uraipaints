...
class RequestApprovalController extends Controller
{
        ...
        return DataTables::of($products)
            ...
            ->addColumn('assign_status', function ($row) {
                if ($row['assign_status'] == 0) {
                    ...
                    $status = '<span class="badge badge-soft-secondary" style="font-size: 12px;">แก้ไขใหม่</span>';
                }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            DB::table('assignments')->insert([
                ...
                'approved_for' => $request->approved_for,
                'assign_is_hot' => $status,
                'assign_shop' => $request->sel_searchShopEdit,
            ...
            DB::table('assignments')->where('id', $request->get_id)
            ->update([
                'parent_id' => 'parent',
                'updated_by' => Auth::user()->id,
    ...
    public function view_comment($id)
    {
        ...
        //-- หา Parent
        $dataassign_parent = array();
        if(!is_null($dataResult->parent_id) && ($dataResult->parent_id != "parent")){
            $assing_parent = DB::table('assignments')
                ->leftjoin('master_objective_assigns', 'master_objective_assigns.id', 'assignments.approved_for')
                ->where('assignments.id', $dataResult->parent_id)
                ->first();
            $dataassign_parent = [
        ...
        return response()->json([
            ...
            'dataassign_parent' => $dataassign_parent,
        ]);
    }
    ...
    public function search_month_requestApprove($fromMonth, $toMonth)
    {
        $list_approval = DB::table('assignments')
            ...
            if($fromMonth != "00"){
                $from = $fromMonth . "-01";
                $list_approval = $list_approval->whereDate('assignments.assign_work_date', '>=', $from);
            }

            if($toMonth != "00"){
                $to = $toMonth . "-31";
                $list_approval = $list_approval->whereDate('assignments.assign_work_date', '<=', $to);
            }

            $list_approval = $list_approval->orderBy('assignments.assign_request_date', 'desc')
            ->groupBy('assignments.id')
            ->get();
