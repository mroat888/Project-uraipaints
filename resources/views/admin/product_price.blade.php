...
<!-- /Breadcrumb -->
                        ...
                        <div class="table-wrap">
                            <div class="hk-pg-header" style="margin-bottom: 30px;">
                                ...
                                <span class="form-inline pull-right">
                                    <form action="{{url('admin/search-product_price')}}" method="POST" enctype="multipart/form-data">
                                        @csrf
                            ...
                            <div class="table-responsive col-md-12 table-color">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$date}}</td>
                                        <td>
                                            <a href="{{url('admin/view_product_price_detail', $value->id)}}">
                                                <img src="{{ isset($value->image) ? asset('public/upload/ProductPrice/' . $value->image) : '' }}" width="100">
                                            </a>
                                        </td>
                                        <td>
                                            @switch($value->status)
                                        ...
                                        <td>
                                                ...
                                                <a href="{{ url('admin/productPrice-gallery', $value->id) }}"
                                                    class="btn btn-icon btn-purple">
                                                    <h4 class="btn-icon-wrap" style="color: white;"><span
                                                            class="material-icons">collections</span></h4>
                                                </a>
                                                <a onclick="edit_modal({{ $value->id }})"
                                                    class="btn btn-icon btn-edit" data-toggle="modal" data-target="#editProductPrice">
                                                    <h4 class="btn-icon-wrap" style="color: white;"><span
                                                        class="material-icons">drive_file_rename_outline</span></h4>
                                                </a>
                                                    @if ($value->status == 0)
                                                    <button type="button" class="btn btn-icon btn-danger" id="btn_price_delete" value="{{ $value->id }}">
                                                        <h4 class="btn-icon-wrap" style="color: white;"><span
    ...
    <!-- /Container -->
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">เพิ่มใบราคา</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="form_insert_product_price" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <label for="name">ชื่อใบราคา</label>
                                <input class="form-control" placeholder="ชื่อใบราคา" name="name" type="text">
                            </div>
                        </div>
     ...
     <!-- Modal Edit -->
     <div class="modal fade" id="editProductPrice" tabindex="-1" role="dialog" aria-labelledby="editProductPrice" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">แก้ไขใบราคา</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="form_update_product_price" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <label for="name">ชื่อใบราคา</label>
                                <input class="form-control" id="get_name" name="name" type="text">
                            </div>
                        </div>
                        <div class="form-group">
    ...
    <!-- Modal Delete -->
    <div class="modal fade" id="ModalPriceDelete" tabindex="-1" role="dialog" aria-labelledby="ModalPriceDelete"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <form id="from_price_delete" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">คุณต้องการลบข้อมูลใบราคาใช่หรือไม่</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body" style="text-align:center;">
                        <h3>คุณต้องการลบข้อมูลใบราคาใช่หรือไม่ ?</h3>
                        <input class="form-control" id="price_id_delete" name="price_id_delete" type="hidden" />
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        $("#form_insert_product_price").on("submit", function (e) {
            e.preventDefault();
            var formData = new FormData(this);
            $.ajax({
                type:'POST',
                url: '{{ url("admin/create_product_price") }}',
                data:formData,
                cache:false,
                contentType: false,
                processData: false,
                success:function(response){
                    ...
                    if(response.status == 200){
                        ...
                        Swal.fire({
                            ...
                            text: "บันทึกข้อมูลใบราคาเรียบร้อยแล้ว",
    ...
    <script>
        ...
        function edit_modal(id) {
            $.ajax({
                type: "GET",
                url: "{!! url('admin/edit_product_price/"+id+"') !!}",
                dataType: "JSON",
                async: false,
                success: function(data) {
                    ...
                    $('#get_name').val(data.dataEdit.name);
                    ...
                    let img_name = '{{ asset('/public/upload/ProductPrice') }}/' + data.dataEdit.image;
...
<script>
    $("#form_update_product_price").on("submit", function (e) {
        e.preventDefault();
        var formData = new FormData(this);
        $.ajax({
            type:'POST',
            url: '{{ url("admin/update_product_price") }}',
            data:formData,
            cache:false,
            contentType: false,
            processData: false,
            success:function(response){
                console.log(response);
                if(response.status == 200){
                    $("#editProductPrice").modal('hide');
                    Swal.fire({
                        icon: 'success',
                        title: 'แก้ไขข้อมูลสำเร็จ',
                        text: "แก้ไขข้อมูลใบราคาเรียบร้อยแล้ว",
                        showConfirmButton: false,
                        timer: 1500
                    });
                    location.reload();
...
<script>
    $(document).on('click', '#btn_price_delete', function() { // ปุ่มลบ Slaplan
            let price_id_delete = $(this).val();
            $('#price_id_delete').val(price_id_delete);
            $('#ModalPriceDelete').modal('show');
        });

        $("#from_price_delete").on("submit", function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        ...
                        text: "ลบข้อมูลใบราคาเรียบร้อยแล้ว",
                        showConfirmButton: false,
                        timer: 1500,
                    });
                    $('#ModalPriceDelete').modal('hide');
                    $('#btn_price_delete').prop('disabled', true);
                    location.reload();
                },
                error: function(response) {
                    console.log("error");
