...
<!-- /Breadcrumb -->
                        ...
                        <div class="table-wrap">
                            <div class="hk-pg-header" style="margin-bottom: 30px;">
                                ...
                                <span class="form-inline pull-right">
                                    <form action="{{url('admin/search-product_age')}}" method="POST" enctype="multipart/form-data">
                                        @csrf
                            ...
                            <div class="table-responsive col-md-12 table-color">
                                ...
                                <tbody>
                                    @foreach ($product_age as $key => $value)
                                    @php
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$date}}</td>
                                        <td>
                                            <a href="{{url('admin/view_product_age_detail', $value->id)}}">
                                                <img src="{{ isset($value->image) ? asset('public/upload/ProductAge/' . $value->image) : '' }}" width="100">
                                            </a>
                                        </td>
                                        <td>
                                            @switch($value->status)
                                        ...
                                        <td>
                                                <a href="{{ url('admin/update-productAge-status-use', $value->id)}}" class="btn btn-icon btn-teal">
                                                    <h4 class="btn-icon-wrap" style="color: white;"><span
                                                        class="material-icons">settings_power</span></h4>
                                                </a>
                                                <a onclick="edit_modal({{ $value->id }})"
                                                    class="btn btn-icon btn-edit" data-toggle="modal" data-target="#editProductAge">
                                                    <h4 class="btn-icon-wrap" style="color: white;"><span
                                                        class="material-icons">drive_file_rename_outline</span></h4>
                                                </a>
                                                    @if ($value->status == 0)
                                                    <button type="button" class="btn btn-icon btn-danger" id="btn_age_delete" value="{{ $value->id }}">
                                                        <h4 class="btn-icon-wrap" style="color: white;"><span
    ...
    <!-- /Container -->
     ...
     <div class="modal fade" id="exampleModalLarge01" tabindex="-1" role="dialog" aria-labelledby="exampleModalLarge01" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id="form_insert_product_age" enctype="multipart/form-data">
     ...
     <!-- Modal Edit -->
     <div class="modal fade" id="editProductAge" tabindex="-1" role="dialog" aria-labelledby="editProductAge" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form id="form_update_product_age" enctype="multipart/form-data">
    ...
    <div class="modal fade" id="ModalAgeDelete" tabindex="-1" role="dialog" aria-labelledby="ModalAgeDelete"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <form id="from_age_delete" enctype="multipart/form-data">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">คุณต้องการลบข้อมูลอายุจัดเก็บสินค้าใช่หรือไม่</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body" style="text-align:center;">
                        <h3>คุณต้องการลบข้อมูลอายุจัดเก็บสินค้าใช่หรือไม่ ?</h3>
                        <input class="form-control" id="age_id_delete" name="age_id_delete" type="hidden" />
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        $("#form_insert_product_age").on("submit", function (e) {
            e.preventDefault();
            ...
            $.ajax({
                type:'POST',
                url: '{{ url("admin/create_product_age") }}',
                data:formData,
                cache:false,
                contentType: false,
                processData: false,
                success:function(response){
                    ...
                    if(response.status == 200){
                        ...
                        Swal.fire({
                            ...
                            text: "บันทึกข้อมูลอายุจัดเก็บสินค้าเรียบร้อยแล้ว",
    ...
    <script>
        ...
        function edit_modal(id) {
            $.ajax({
                type: "GET",
                url: "{!! url('admin/edit_product_age/"+id+"') !!}",
                dataType: "JSON",
                async: false,
...
<script>
    $("#form_update_product_age").on("submit", function (e) {
        e.preventDefault();
        var formData = new FormData(this);
        $.ajax({
            type:'POST',
            url: '{{ url("admin/update_product_age") }}',
            data:formData,
            cache:false,
            contentType: false,
            processData: false,
            success:function(response){
                console.log(response);
                if(response.status == 200){
                    $("#editProductAge").modal('hide');
                    Swal.fire({
                        icon: 'success',
                        title: 'แก้ไขข้อมูลสำเร็จ',
                        text: "แก้ไขข้อมูลอายุจัดเก็บสินค้าเรียบร้อยแล้ว",
                        showConfirmButton: false,
                        timer: 1500
                    });
                    location.reload();
...
<script>
    $(document).on('click', '#btn_age_delete', function() { // ปุ่มลบ Slaplan
            let age_id_delete = $(this).val();
            $('#age_id_delete').val(age_id_delete);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        ...
                        text: "ลบข้อมูลอายุจัดเก็บสินค้าเรียบร้อยแล้ว",
                        showConfirmButton: false,
                        timer: 1500,
                    });
                    $('#ModalAgeDelete').modal('hide');
                    $('#btn_age_delete').prop('disabled', true);
                    location.reload();
                },
                error: function(response) {
                    console.log("error");
