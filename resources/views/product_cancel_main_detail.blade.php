  ...
  .slide-navButton.nav-out .owl-carousel .owl-nav i {
    ...
    line-height: 43px; }
         ...
         <div class="hk-pg-header mb-10">
            <div class="topichead-bgred"><i data-feather="clipboard"></i> รายละเอียดสินค้ายกเลิก</div>
            <div class="content-right d-flex">
                @if (Auth::user()->status == 1)
                    <a href="{{url('product_cancel')}}" type="button" class="btn btn-secondary btn-rounded ml-2"> ย้อนกลับ</a>
                @elseif (Auth::user()->status == 2)
                    <a href="{{url('lead/product_cancel')}}" type="button" class="btn btn-secondary btn-rounded ml-2"> ย้อนกลับ</a>
                @elseif (Auth::user()->status == 3)
                <a href="{{url('head/product_cancel')}}" type="button" class="btn btn-secondary btn-rounded ml-2"> ย้อนกลับ</a>
                @elseif (Auth::user()->status == 4)
                <a href="{{url('admin/product_cancel')}}" type="button" class="btn btn-secondary btn-rounded ml-2"> ย้อนกลับ</a>
                @endif
                ...
                <div class="col-md-6">
                    <div class="partnerGal-slide detail_slide" style="border: 1px solid #ddd;">
                        <div id="big" class="owl-carousel owl-theme big-img slider">
                            <a class="item" data-fancybox="gallery" href="{{ asset('public/upload/ProductCancel/' . $data_product->image)}}"><img src="{{ asset('public/upload/ProductCancel/' . $data_product->image)}}"></a>
                        </div>
                    </div>
                </div>
